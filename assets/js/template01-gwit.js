(function(){
  ...
  if(!prefersReduced && 'IntersectionObserver' in window){
    ...
    document.querySelectorAll('.gwit-reveal, .gwit-reveal-stagger').forEach(function(el){
  ...
  if(kpiElements.length && 'IntersectionObserver' in window && !prefersReduced){
    var kpiAnimated=new WeakSet();

    var animateCount=function(el){
      if(kpiAnimated.has(el)) return;
      kpiAnimated.add(el);
      ...
      var formatNumber=function(num){
        var str=decimals>0?num.toFixed(decimals):Math.round(num).toString();
        if(separator){
          var parts=str.split('.');
  ...
  /* ============================
     ...
     ============================ */
  var nlForm=document.getElementById('footerNewsletter');
  if(nlForm){
