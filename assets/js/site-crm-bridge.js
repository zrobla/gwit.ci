(function () {
  if (window.__gwitSiteCrmBridgeLoaded) {
    return;
  }
  window.__gwitSiteCrmBridgeLoaded = true;
  ...
  function normalizeSegment(value) {
    ...
    if (text.indexOf("benchmark") >= 0 || text.indexOf("qos") >= 0 || text.indexOf("qoe") >= 0) {
      return "benchmarking-qos";
    }
    if (text.indexOf("ingenier") >= 0 || text.indexOf("plan directeur") >= 0 || text.indexOf("ran") >= 0) {
      return "ingenierie-telecom";
    }
    if (
      text.indexOf("deploi") >= 0 ||
      text.indexOf("installation") >= 0 ||
      text.indexOf("rollout") >= 0 ||
      text.indexOf("acceptance") >= 0
    ) {
      return "deploiement-installations";
    }
    if (
      text.indexOf("soc") >= 0 ||
      text.indexOf("cyber") >= 0 ||
      text.indexOf("it") >= 0 ||
      text.indexOf("pra") >= 0 ||
      text.indexOf("pca") >= 0
    ) {
    ...
    if (text.indexOf("conseil") >= 0 || text.indexOf("formation") >= 0 || text.indexOf("support") >= 0) {
      return "conseil-formation";
    }
    if (text.indexOf("regulat") >= 0) {
      return "regulation";
    }
  ...
  function normalizeChannel(value) {
    ...
    if (text.indexOf("visio") >= 0 || text.indexOf("video") >= 0) {
      return "visio";
    }
    ...
    if (text.indexOf("site") >= 0 || text.indexOf("meeting") >= 0 || text.indexOf("reunion") >= 0) {
      return "sur-site";
    }
    return "";
  }
  ...
  function init() {
    ...
    bindInboundForm(document.getElementById("c-form"), "contact", {
      fields: {
        ...
        segment: ["serviceNeed", "profile", "segment"],
        stage: ["profile", "organisation", "role"],
        intent: ["serviceNeed", "intent"],
        channel_preference: ["contactPref", "channel", "channel_preference"],
        message: ["context", "message"],
        region: ["location", "region"],
        objective: ["timeline", "timeline_detail"],
      },
    });
    ...
    bindInboundForm(document.getElementById("briefingForm"), "contact", {
      fields: {
        ...
        phone: ["phone", "contactPhone"],
        email: ["email", "workEmail"],
        stage: ["organisation", "role"],
        intent: ["topics", "serviceNeed", "intent"],
        interests: ["topics", "data"],
